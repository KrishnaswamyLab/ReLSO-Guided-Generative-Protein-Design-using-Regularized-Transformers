...
def plot_embedding(embeddings, fitness, wandb_logger, save_path):
    ...
    if wandb_logger:
        wandb_logger.experiment.log({f'Original Embedding': wandb.Image(plt)})
...
def plot_multiple_optim_traj(embeddings, fitness, optim_embeddings_list, 
                            traj_names, save_path, run_indx, wandb_logger):
    """[summary]
    ...
    if wandb_logger:
        wandb_logger.experiment.log({f'LSO Optimization Trajectories seed={run_indx}': wandb.Image(plt)})
...
def plot_landscape_3d(coords, fitness, save_path, wandb_logger=None):
    """
    ...
    if wandb_logger:
        wandb_logger.experiment.log({f'3D Embedding': wandb.Image(plt)})
    plt.close()
...
def plot_embedding_end_points(embeddings, fitness, optim_embeddings, algo_name, save_path, wandb_logger=None):
    """
    ...
    if wandb_logger:
        wandb_logger.experiment.log({f'LSO ending points {algo_name}': wandb.Image(plt)})
